$(document).ready(function () {
	...
	if ($("body").width() > 1025) {
		$(".marks-item, .allmarks-item").on("click", function () {
			...
			if (
				$("#marks2").find("option[value='" + $(this).attr("data-val") + "']")
					.length
			) {
	...
	$(".examples-item").each(function () {
		...
		$(this).hover(
			function () {
				var height_new =
					$(this).find(".examples-item__body__hidden").outerHeight() +
					height +
					25;
		...
		.each(function (a, b) {
			if (!$(this).is(":disabled")) {
				if ($(this).val().length < 1) {
					...
					$("#select2-" + id_val + "-container")
						.closest(".select2-container")
						.addClass("not_validate");
...
$("input[type=checkbox]").on("change", function () {
	$(".c-check .check").css(
		"box-shadow",
		"0px 0px 22.5px 2.5px rgba(1, 21, 45, 0.25)"
	);
});
